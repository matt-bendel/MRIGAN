...
def get_gen_supervised(args):
    ...
    checkpoint_file_gen = pathlib.Path(
        f'/home/bendel.8/Git_Repos/full_scale_mrigan/MRIGAN/trained_models/adler/generator_model.pt')
    checkpoint_gen = torch.load(checkpoint_file_gen, map_location=torch.device('cuda'))

    checkpoint_file_dis = pathlib.Path(
        f'/home/bendel.8/Git_Repos/full_scale_mrigan/MRIGAN/trained_models/adler/discriminator_model.pt')
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            for j in range(args.num_iters_discriminator):
                ...
                # Adversarial loss
                d_loss = torch.mean(fake_pred) - torch.mean(real_pred) + lambda_gp * gradient_penalty + 0.001 * torch.mean(real_pred ** 2)
            ...
            with torch.no_grad():
                ...
                for k in range(ims.shape[0]):
                    ...
                    if i + 1 == 1 and k == 2:
                        ...
                        plt.savefig(f'std_dev_gen_adler_{args.z_location}.png')
                        plt.close()

                        plt.figure()
                        plt.imshow(np.abs(output), cmap='gray')
                        plt.savefig(f'temp_gen_out_adler_{args.z_location}.png')
                        plt.close()

                        plt.figure()
                        plt.imshow(np.abs(target), cmap='gray')
                        plt.savefig('temp_gen_targ_adler.png')
                        plt.close()
