...
def get_cfid_torch(y_predict, x_true, y_true, np_inv=False, mat=False):
    ...
    dtype = np.float32
    inv_c_x_true_x_true = torch.tensor(np.linalg.pinv(x_t_x.cpu().numpy(), rcond=10.*2048*np.finfo(dtype).eps)).to(y_true.device) if np_inv else torch.linalg.pinv(x_t_x)
