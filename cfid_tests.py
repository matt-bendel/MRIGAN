...
if __name__ == '__main__':
    mat_test = False
    recon_embeds = torch.load('image_embeds_600.pt').to(dtype=torch.float64)
    cond_embeds = torch.load('cond_embeds_600.pt').to(dtype=torch.float64)
    gt_embeds = torch.load('true_embeds_600.pt').to(dtype=torch.float64)
