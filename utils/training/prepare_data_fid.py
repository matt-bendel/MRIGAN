    ...
    def __call__(self, kspace, target, attrs, fname, slice):
        ...
        if self.args.inpaint:
            ...
            if self.args.patches:
                new_cond_im = torch.zeros(self.args.num_patches ** 2, 3, 128 // (self.args.num_patches),
                                          128 // (self.args.num_patches))
                new_ref_im = torch.zeros(self.args.num_patches ** 2, 3, 128 // (self.args.num_patches),
                                         128 // (self.args.num_patches))
                col = 0
                for i in range(self.args.num_patches ** 2):
                    ...
                    new_cond_im[i, :, :, :] = cond_im[:, ind * 128 // (self.args.num_patches):(ind + 1) * 128 // (
                        self.args.num_patches), col:col + 128 // (self.args.num_patches)]
                    new_ref_im[i, :, :, :] = ref_im[:, ind * 128 // (self.args.num_patches):(ind + 1) * 128 // (
                        self.args.num_patches), col:col + 128 // (self.args.num_patches)]
        ...
        if self.args.patches:
            new_cond_im = torch.zeros(self.args.num_patches**2, 3, 128 // (self.args.num_patches), 128 // (self.args.num_patches))
            new_ref_im = torch.zeros(self.args.num_patches**2, 3, 128 // (self.args.num_patches), 128 // (self.args.num_patches))
            col = 0
            for i in range(self.args.num_patches**2):
                ind = i % self.args.num_patches
                if i < self.args.num_patches and i % self.args.num_patches == 0:
